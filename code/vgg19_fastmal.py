...
class Vgg19 :
    ...
    def build_avg_pool2(self, rgb, train_mode=None):
        ...
        print([self.imsize, self.imsize,1])
        assert red.get_shape().as_list()[1:] == [self.imsize, self.imsize, 1]
        assert green.get_shape().as_list()[1:] == [self.imsize, self.imsize, 1]
        assert blue.get_shape().as_list()[1:] == [self.imsize, self.imsize, 1]
        bgr = tf.concat(axis=3, values=[
    ...
    def get_conv_var(self, filter_size, in_channels, out_channels, name):
        initial_value = tf.compat.v1.truncated_normal([filter_size, filter_size, in_channels, out_channels], 0.0, 0.001)
        filters = self.get_var(initial_value, name, 0, name + "_filters")

        initial_value = tf.compat.v1.truncated_normal([out_channels], .0, .001)
        biases = self.get_var(initial_value, name, 1, name + "_biases")

        return filters, biases

    def get_fc_var(self, in_size, out_size, name):
        initial_value = tf.compat.v1.truncated_normal([in_size, out_size], 0.0, 0.001)
        weights = self.get_var(initial_value, name, 0, name + "_weights")

        initial_value = tf.compat.v1.truncated_normal([out_size], .0, .001)
        biases = self.get_var(initial_value, name, 1, name + "_biases")
